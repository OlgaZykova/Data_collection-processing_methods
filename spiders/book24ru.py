...
from some_funcs import make_link, current_page_number
...
class Book24ruSpider(scrapy.Spider):
    ...
    def parse(self, response: HtmlResponse):
        # добавила ограничение по количеству перебираемых страниц - запускала без ограничения - это СЛИШКОМ долго
        max_page_number = 10
        ...
        # вот тут подключаю проверку условия на номер текущей страницы
        if response.xpath("//div[@class='product-list__item']").extract() and current_page_number(response.url,
                                                                                                  self.name) <= max_page_number or response.url in self.start_urls:
    ...
    def book_parse(self, response: HtmlResponse):
        ...
        item = BooksearchItem(title=book_title, authors=book_authors, link=book_link, price=book_price,
