...
class LabirintruSpider(scrapy.Spider):
    ...
    def parse(self, response: HtmlResponse):
        # добавила ограничение на число перебираемых страниц
        max_page_number = 10
        next_page = make_link(response.url, self.name)
        #тут есть проверка на номер страницы
        if response.xpath(
        ...
        books_links = response.xpath(
            "//div[contains(@data-title,'Детская художественная литература') and @class = 'products-row ']//a[@class = 'product-title-link']/@href").extract()
